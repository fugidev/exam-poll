...
const ShareLink: React.FC = () => {
  const [copyConfirmationTimeout, setCopyConfirmationTimeout] = useState<ReturnType<typeof setTimeout>>(null);
  ...
  const copyURL: React.MouseEventHandler<HTMLInputElement|HTMLAnchorElement> = async (e) => {
    ...
    navigator.clipboard.writeText(input.value).then(() => {
      if (copyConfirmationTimeout !== null) clearTimeout(copyConfirmationTimeout);
    ...
    <div>
      ...
      <div className={styles.sharePollWrapper}>
        <a
          ...
          title={copyConfirmationTimeout ? "Link copied" : "Copy link to clipboard"}
          onClick={copyURL}
          style={{
